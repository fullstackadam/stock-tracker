class UserStocksController < ApplicationController
  ...
  before_action :can_create_stock?, only: [:create]
  ...
  def destroy
    @user_stock.destroy
    respond_to do |format|
      format.html { redirect_to my_portfolio_path, notice: "#{@user_stock.stock.ticker} stock was removed from portfolio." }
  ...
  private
    ...
    def set_user_stock
      @user_stock = UserStock.where(stock_id: params[:id], user_id: current_user.id)[0]
    end
    ...
    def can_create_stock?
      if !current_user.can_add_stock?(params[:stock_ticker])
        redirect_to my_portfolio_path, danger: current_user.under_stock_limit? ? 'Stock cannot be added twice.' : 'Cannot add more than 10 stocks'
      end
    end
end
